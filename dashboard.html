...
module.exports = async (req, res) => {
  ...
  // Demo mode: no database — return demo report with sample feedback
  if (!process.env.DATABASE_URL) {
    const doctor = FALLBACK_DOCTORS.find(d => d.id === doctorId) || FALLBACK_DOCTORS[0];
    return res.json({
      doctor,
      feedbacks: [
        { id: 1, rating: 5, comments: 'Excellent doctor, very thorough.', treatment_result: 'Cured', created_at: new Date().toISOString(), patient_name: 'Sample Patient' },
        { id: 2, rating: 4, comments: 'Good experience overall.', treatment_result: 'Improved', created_at: new Date().toISOString(), patient_name: 'Demo User' },
      ],
      stats: { total_feedback: 2, average_rating: 4.5 },
      demo: true
    });
  }
